...
const changePassword = async (req, res) => {
  ...
  try {
    ...
    await changePasswordModel(userId, hashedPassword);
...
const updateUser = async (req, res) => {
  ...
  try {
    ...
    if (file) {
      newPhotoUrl = await uploadProfilePhotoToSpaces(file);
      // Delete the old photo if it exists
      if (oldPhotoUrl) {
        const deleteResult = await deleteFileFromSpaces(oldPhotoUrl);
